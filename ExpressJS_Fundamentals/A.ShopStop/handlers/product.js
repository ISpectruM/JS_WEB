  ...
  if (req.pathname === '/product/add' && req.method === 'GET') {
    ...
    fs.readFile(filePath, (err, data) => {
      ...
      Category.find().then((categories) => {
        categoryMenu = '<select class="input-field" name="category"><option>-- Choose product category --</option>'
        for (const category of categories) {
          categoryMenu += `<option value=${category._id}>${category.name}</option>`
        }
        categoryMenu += '</select>'
        data = data.toString().replace('{{categories}}', categoryMenu)
        res.write(data)
        res.end()
      })
    })
